...
```js
const { tailwindLayouts } = require('tailwind-every-layout')
...
You can do this by applying `max-is-measure` along with the `prose` class to the prose container.

```html
<div class="prose max-is-measure">
  <!-- prose content -->
</div>
```
...
If for some reason you want to override the measure for a specific element, you can always use the `max-is-none` class.
...
```
Also, if you want to use min/max width/height classes, instead of "w" and "h" you have to use "is" and "bs".
For Example:
```js
